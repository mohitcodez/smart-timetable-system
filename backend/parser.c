/*
 * parser.c  –  Lightweight JSON parser for the Smart Timetable input.
 *
 * Reads a JSON file of the form:
 * {
 *   "teachers":  [ {"teacher_id":0, "teacher_name":"...", ...}, ... ],
 *   "rooms":     [ {"room_id":0,    "room_name":"...", "capacity":30}, ... ],
 *   "subjects":  [ {"subject_id":0, "subject_name":"...",
 *                   "teacher_id":0, "lectures_per_week":3}, ... ],
 *   "batches":   [ {"batch_id":0,   "batch_name":"...", "student_count":25}, ... ]
 * }
 *
 * This parser is intentionally minimal – it does NOT handle every possible
 * JSON edge-case but is sufficient for the well-structured input files used
 * by this project.  It uses sscanf / strstr scanning rather than a full
 * lexer so as to keep the C code self-contained and dependency-free.
 */
...
/* Locate the next occurrence of key "key" in *p, advancing *p past it.
...
/* Read a quoted string from *p into buf (max bufLen-1 chars).
   *p is advanced past the closing quote. */
