...
int main(int argc, char *argv[])
{
    ...
    if (sessionCount == 0) {
        /* No sessions – emit empty timetable */
        printf("{\"timetable\":[]}\n");
        return 0;
    }
    ...
    /* 5. Try greedy coloring first */
    int colorsUsed = greedyColoring(&g);

    /* 6. If greedy exceeds available slots, fall back to backtracking */
