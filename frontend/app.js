// app.js - handles all CRUD operations and navigation for the timetable dashboard
...
function nextId(arr, field) {
    ...
    return Math.max.apply(null, arr.map(function(x) { return x[field]; })) + 1;
}
...
function renderTeachers() {
    ...
    tbody.innerHTML = db.teachers.map(function(t, i) {
        return '<tr>' +
            ...
            '<td>' +
                '<button class="btn btn-secondary" style="padding:4px 10px;font-size:.78rem;" onclick="app.editTeacher(' + t.teacher_id + ')">Edit</button> ' +
                '<button class="btn btn-danger" style="padding:4px 10px;font-size:.78rem;" onclick="app.deleteTeacher(' + t.teacher_id + ')">Delete</button>' +
            '</td>' +
            '</tr>';
    }).join('');
}
...
function editTeacher(id) {
    var t = db.teachers.find(function(x) { return x.teacher_id === id; });
    if (t) openTeacherModal(t);
}
function deleteTeacher(id) {
    if (!confirm('Delete this teacher?')) return;
    db.teachers = db.teachers.filter(function(x) { return x.teacher_id !== id; });
    save(KEYS.teachers, db.teachers);
    renderTeachers();
    updateStats();
...
function renderRooms() {
    ...
    tbody.innerHTML = db.rooms.map(function(r, i) {
        return '<tr>' +
            ...
            '<td>' +
                '<button class="btn btn-secondary" style="padding:4px 10px;font-size:.78rem;" onclick="app.editRoom(' + r.room_id + ')">Edit</button> ' +
                '<button class="btn btn-danger" style="padding:4px 10px;font-size:.78rem;" onclick="app.deleteRoom(' + r.room_id + ')">Delete</button>' +
            '</td>' +
            '</tr>';
    }).join('');
}
...
function editRoom(id) {
    var r = db.rooms.find(function(x) { return x.room_id === id; });
    if (r) openRoomModal(r);
}
function deleteRoom(id) {
    if (!confirm('Delete this room?')) return;
    db.rooms = db.rooms.filter(function(x) { return x.room_id !== id; });
    save(KEYS.rooms, db.rooms);
    renderRooms();
    updateStats();
...
function renderSubjects() {
    ...
    tbody.innerHTML = db.subjects.map(function(s, i) {
        var teacher = db.teachers.find(function(t) { return t.teacher_id === s.teacher_id; });
        return '<tr>' +
            ...
            '<td>' +
                '<button class="btn btn-secondary" style="padding:4px 10px;font-size:.78rem;" onclick="app.editSubject(' + s.subject_id + ')">Edit</button> ' +
                '<button class="btn btn-danger" style="padding:4px 10px;font-size:.78rem;" onclick="app.deleteSubject(' + s.subject_id + ')">Delete</button>' +
            '</td>' +
            '</tr>';
    }).join('');
}
...
function editSubject(id) {
    var s = db.subjects.find(function(x) { return x.subject_id === id; });
    if (s) openSubjectModal(s);
}
function deleteSubject(id) {
    if (!confirm('Delete this subject?')) return;
    db.subjects = db.subjects.filter(function(x) { return x.subject_id !== id; });
    save(KEYS.subjects, db.subjects);
    renderSubjects();
    updateStats();
...
function renderBatches() {
    ...
    tbody.innerHTML = db.batches.map(function(b, i) {
        return '<tr>' +
            ...
            '<td>' +
                '<button class="btn btn-secondary" style="padding:4px 10px;font-size:.78rem;" onclick="app.editBatch(' + b.batch_id + ')">Edit</button> ' +
                '<button class="btn btn-danger" style="padding:4px 10px;font-size:.78rem;" onclick="app.deleteBatch(' + b.batch_id + ')">Delete</button>' +
            '</td>' +
            '</tr>';
    }).join('');
}
...
function editBatch(id) {
    var b = db.batches.find(function(x) { return x.batch_id === id; });
    if (b) openBatchModal(b);
}
function deleteBatch(id) {
    if (!confirm('Delete this batch?')) return;
    db.batches = db.batches.filter(function(x) { return x.batch_id !== id; });
    save(KEYS.batches, db.batches);
    renderBatches();
    updateStats();
...
// expose to global scope (used by inline onclick handlers and timetable.js)
window.app = {
