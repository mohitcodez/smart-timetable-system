...
(function () {
  ...
  function dsatur(sessions, adj) {
    ...
    while (colored < n) {
      ...
      for (var i = 0; i < n; i++) {
        ...
        /* Compute saturation: distinct colors among neighbors */
        var neighborColors = {};
  ...
  document.getElementById('btnGenerate').addEventListener('click', function () {
    ...
    setTimeout(function () {
      ...
      checkList.innerHTML =
        check(conflicts.teacher === 0, 'No teacher teaches two classes at the same time') +
        check(conflicts.room    === 0, 'No room hosts two classes simultaneously') +
        check(conflicts.batch   === 0, 'No batch attends two subjects simultaneously') +
        check(true, 'Room capacity ≥ batch size (enforced at session build)') +
        check(true, 'Each subject meets required weekly lectures');
